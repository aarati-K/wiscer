...
void ChainedHashmap::storeHashmap() {
    stack<ulong> chain;
    ofstream f("hashmap.out");
    if (!f.is_open()) {
        cout << "Could not open hashmap.out file" << endl;
        return;
    }
    KV* ptr;
    for (ulong i=0; i<hmsize; i++) {
        ptr = dict[i];
        while (ptr != NULL) {
            chain.push(ptr->key);
            ptr = ptr->next;
...
inline void ChainedHashmap::_fetch(HashmapReq *r) {
    ...
    while (ptr && ptr->key != r->key) {
        displacement += 1;
        if (ptr->accesses < min_access_entry->accesses) {
    ...
    if (ptr->accesses > min_access_entry->accesses) {
        ...
        min_access_entry->key = r->key;

        ulong val = min_access_entry->value;
        min_access_entry->value = ptr->value;
        ptr->value = val;

        uint accesses = min_access_entry->accesses;
        min_access_entry->accesses = ptr->accesses;
        ptr->accesses = accesses;
    }
    displacement += 1;
    r->value = ptr->value;
    numReqs += 1;
